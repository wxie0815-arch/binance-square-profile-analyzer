---
name: binance-square-profile-analyzer
description: 抓取并分析币安广场（Binance Square）用户主页内容的Skill。支持智能用户搜索、全量帖子抓取、深度内容分析（写作风格、活跃度、互动数据、创作偏好）。无需API Key或登录凭证，直接运行。
---
...
脚本路径为 `scripts/binance_profile_analyzer.py`，相对于技能根目录。
...
```bash
python3 scripts/binance_profile_analyzer.py analyze "CZ" --output ./analysis_report
```
